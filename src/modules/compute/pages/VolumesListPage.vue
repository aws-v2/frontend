    ...
    <div class="min-h-screen bg-white text-[#16191f] font-urbanist p-8 md:p-16 pt-24 relative overflow-hidden">
        ...
        <div class="relative z-10 max-w-7xl mx-auto space-y-12">
            <nav class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] mb-4 text-[#879196]">
                ...
                <div class="w-1 h-1 bg-[#eaeded] rounded-full"></div>
                <span class="text-[#232f3e]">VOLUMES_INVENTORY</span>
            </nav>

            <div class="flex justify-between items-end border-b-4 border-[#232f3e] pb-10">
                <h1 class="text-6xl font-black text-[#232f3e] tracking-tighter uppercase leading-none">Block_Volumes
                </h1>
                <button @click="router.push({ name: 'create-volume' })"
                    class="px-12 py-5 bg-blue-600 text-white text-[11px] font-black uppercase tracking-[0.3em] hover:bg-[#232f3e] transition-all">Create_Volume</button>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-[#fafafa] border-b-2 border-[#232f3e]">
                            ...
                            <th class="px-8 py-6 text-[10px] font-black text-[#232f3e] uppercase tracking-widest">
                                Capacity</th>
                            <th class="px-8 py-6 text-[10px] font-black text-[#232f3e] uppercase tracking-widest">State
                            </th>
                            <th
                    ...
                    <tbody class="divide-y-2 divide-[#eaeded]">
                        <tr v-for="vol in computeStore.volumes" :key="vol.id"
                            ...
                            <td class="px-8 py-6">
                                <span
                                    class="text-[10px] font-black px-4 py-1.5 bg-emerald-500/5 border-2 border-emerald-500/20 text-emerald-600 uppercase tracking-widest">{{
                                    vol.state }}</span>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <button
                                    class="text-[10px] font-black text-[#879196] hover:text-red-600 transition-colors uppercase tracking-widest">Decommission</button>
                            </td>
                        </tr>
                        <tr v-if="computeStore.volumes.length === 0">
                            <td colspan="4" class="px-8 py-20 text-center opacity-40">
                                <p class="text-[11px] font-black uppercase tracking-[0.4em] text-[#879196]">
                                    No_Volumes_Detected</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
