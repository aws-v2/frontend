  ...
  <div class="min-h-screen pb-24 bg-white font-urbanist selection:bg-[#ff9900]/30">
    ...
    <div class="relative z-10 max-w-[1600px] mx-auto px-8 md:px-16 pt-24 pb-20">
      ...
      <div v-else-if="error" class="py-32 text-center">
        ...
        <p class="text-[10px] text-[#879196] font-black uppercase tracking-widest">
          Could not load game manifest.
        </p>
      </div>
      ...
      <template v-else-if="manifest">
        ...
        <header class="mb-14">
          ...
          <p class="text-[#545b64] font-medium text-sm">
            Build configuration and deployment manifest.
          </p>
          ...
          <div class="mt-6">
            <button
              @click="playGame"
              :disabled="starting"
              class="inline-flex items-center gap-3 px-6 py-3 bg-[#ff9900] text-[#232f3e] font-black text-xs uppercase tracking-widest border-2 border-[#ff9900] hover:bg-[#232f3e] hover:text-[#ff9900] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div class="border-2 border-[#232f3e] overflow-hidden">
          <div class="px-8 py-5 bg-[#fafafa] border-b-2 border-[#eaeded] flex items-center justify-between">
            <h2 class="text-sm font-black text-[#232f3e] uppercase tracking-tighter">
              Game Manifest
            </h2>
            <span class="text-[9px] font-black text-[#879196] uppercase tracking-[0.3em]">
              Build Config
            </span>
          </div>
          ...
          <div class="p-8">
            ...
            <!-- Headless Binary -->
            <div class="mb-8">
              <p class="text-[10px] font-black text-[#879196] uppercase tracking-widest mb-2">
                Headless Binary
              </p>
            ...
            <div v-if="manifest.required_files?.length || manifest.RequiredFiles?.length" class="mb-8">
              <p class="text-[10px] font-black text-[#879196] uppercase tracking-widest mb-3">
                Required Files
              </p>
              <div class="space-y-2">
                <div
                  v-for="(file, i) in (manifest.required_files || manifest.RequiredFiles)"
                  :key="i"
                  class="flex items-center gap-3 px-4 py-3 border border-[#eaeded] bg-[#fafafa]"
                >
                  <svg class="w-4 h-4 text-[#ff9900]" fill="none" viewBox="0 0 24 24">
            ...
            <div v-if="manifest.ports || manifest.Ports" class="mb-8">
              ...
              <div class="flex flex-wrap gap-2">
                <template v-if="Array.isArray(manifest.ports || manifest.Ports)">
                  <span
                    v-for="(port, i) in (manifest.ports || manifest.Ports)"
                    :key="i"
                ...
                <template v-else>
                  <span
                    v-for="(val, proto) in (manifest.ports || manifest.Ports)"
                    :key="proto"
                    class="font-mono text-xs px-3 py-1.5 bg-[#232f3e] text-[#ff9900] font-bold"
                  >
                    {{ proto }}:{{ val }}
            ...
            <div>
              ...
              <pre class="bg-[#fafafa] border border-[#eaeded] p-4 text-xs font-mono text-[#545b64] overflow-auto max-h-96 whitespace-pre-wrap">
