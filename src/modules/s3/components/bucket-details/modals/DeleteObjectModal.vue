...
const confirmationText = computed(() => {
    if (props.objects.length === 1 && props.objects[0]) {
        return props.objects[0].key.split('/').pop() || props.objects[0].key
    }
    return 'permanently delete'
})

const isConfirmationValid = computed(() => {
    return confirmationInput.value === confirmationText.value
})
...
const handleDelete = async () => {
    ...
    try {
        await s3Store.deleteFiles(props.bucketName, props.objects)
        toastStore.addToast(`Successfully deleted ${props.objects.length} item(s)`, 'success')
        emit('success')
        handleClose()
    } catch (error: any) {
        toastStore.addToast('Failed to delete objects', 'error')
    } finally {
        ...
        <div
            ...
            <div class="p-10 overflow-y-auto max-h-[70vh] relative z-10 bg-white">
                    ...
                    <div class="flex items-start gap-6">
                        ...
                        <ul
                            ...
                            <li
                                ...
                                Deleting objects is irreversible.</li>
                            <li>Objects will be permanently removed from bucket <strong>{{ bucketName }}</strong>.</li>
                            <li v-if="objects.length > 5">Total objects to delete: <strong>{{ objects.length }}</strong>
                            </li>
                        </ul>
                    </div>
                </div>

                <!-- Object List (if a few) -->
                <div v-if="objects.length > 0 && objects.length <= 5" class="mb-8 border-2 border-[#eaeded] p-6 italic">
                    <h4 class="text-[10px] font-black uppercase tracking-widest text-[#545b64] mb-4">Target Objects</h4>
                    <ul class="space-y-2">
                        <li v-for="obj in objects" :key="obj.file_id"
                            class="text-xs font-black text-[#232f3e] truncate bg-[#fafafa] px-3 py-1.5 border border-[#eaeded]">
                            {{ obj.key }}
                        </li>
                    </ul>
                </div>
                ...
                <div class="border-2 border-[#eaeded] p-8 bg-[#fafafa] shadow-inner italic">
                    <h3 class="text-xl font-black text-[#232f3e] mb-2 uppercase tracking-tighter italic">Confirm
                        Permanent Delete</h3>
                    <p class="text-[11px] text-[#545b64] mb-6 font-bold uppercase tracking-widest leading-relaxed">
                        To authorize the permanent destruction of these objects, enter precisely:
                        <span
                    ...
                    <input v-model="confirmationInput" type="text" :placeholder="confirmationText.toUpperCase()"
