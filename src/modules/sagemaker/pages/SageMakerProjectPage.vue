...
const buildDefaultEdges = (ns: PipelineNode[]): PipelineEdge[] =>
  ns.slice(0,-1).map((n,i) => ({ id:`edge-${i}`, fromNodeId:n.id, toNodeId:ns[i+1].id }))
...
const saveProject = async () => {
  ...
  try {
    await apiClient.post(`/llm/training/jobs`, project.value)
  } catch (e) {
...
const updateProject = async () => {
  if (!project.value) return
  try {
    await apiClient.put(`/llm/training/jobs/${project.value.id}?session=${project.value.session_id}`, project.value)
  } catch (e) {
    console.error('Failed to save project', e)
  }
}
...
const onCanvasDrop = (e: DragEvent) => {
  ...
  const newNode: PipelineNode = {
    ...
    default_run: type !== 'deploy', // deploy is protected by default
  }
...
const onPortConnect = (_e: MouseEvent, toNodeId: string) => {
  if (!drawingEdge.value || !drawingFromNodeId.value || !project.value) return
  if (drawingFromNodeId.value === toNodeId) return
  const exists = project.value.edges.some(e => e.fromNodeId === drawingFromNodeId.value && e.toNodeId === toNodeId)
  if (!exists) { project.value.edges.push({ id:`edge-${Date.now()}`, fromNodeId:drawingFromNodeId.value, toNodeId }); saveProject() }
}
...
// ─── VM mode ─────────────────────────────────────────────────────────────────
const setVmMode = (nodeId: string, mode: 'shared'|'dedicated') => {
  const node = project.value?.nodes.find(n => n.id === nodeId)
  if (!node) return
  node.vmMode = mode; node.vm = mode === 'dedicated' ? `vm-${node.type}-${nodeId.slice(-4)}` : 'shared-01'
  saveProject()
}
...
const scriptForm         = ref({ name:'', path:'', routeTo:'' })
const availableCleanerScripts = computed(() => nodes.value.find(n => n.type==='clean')?.scripts.map(s=>s.name) ?? [])
...
const addScript = (nodeId: string) => {
  ...
  scriptForm.value={name:'',path:'',routeTo:''}; showScriptModal.value=true
}
...
const openScriptProps = (nodeId: string, scriptId: string) => {
  ...
  scriptForm.value={name:script.name,path:script.path,routeTo:script.routeTo??''}; showScriptModal.value=true
}
const saveScript = () => {
  ...
  if (editingScript.value) {
    const s = node.scripts.find(s => s.id === editingScript.value!.id)
    if (s) { s.name=scriptForm.value.name; s.path=scriptForm.value.path; s.routeTo=scriptForm.value.routeTo||undefined }
  } else {
    node.scripts.push({ id:`script-${Date.now()}`, name:scriptForm.value.name, path:scriptForm.value.path, routeTo:scriptForm.value.routeTo||undefined })
  }
  updateProject(); showScriptModal.value=false
  // saveProject(); showScriptModal.value=false
}
  ...
  <div class="flex flex-col h-screen bg-white font-urbanist overflow-hidden text-[#232f3e]">
    ...
    <div v-if="notFound" class="flex-1 flex items-center justify-center">
      <div class="text-center">
        ...
        <button @click="router.push('/sagemaker')" class="px-6 py-3 bg-[#ff9900] text-white text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#ec7211] transition-all">← Back to SageMaker</button>
      </div>
    </div>
    ...
    <template v-if="project">
      ...
      <header class="flex items-center gap-3 h-12 px-4 bg-white border-b-2 border-[#eaeded] flex-shrink-0 z-20">
        ...
        <nav class="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.15em]">
          <button @click="router.push('/sagemaker')" class="text-[#879196] hover:text-[#ff9900] transition-colors bg-transparent border-none cursor-pointer font-black uppercase tracking-[0.15em]">sagemaker</button>
        ...
        <!-- Stat pills -->
        <div class="flex items-center gap-2 ml-2">
          <span class="px-2 py-0.5 text-[9px] font-black uppercase tracking-widest border border-[#eaeded] text-[#879196]">
            runs <span class="text-[#232f3e]">{{ project.runs?.length }}</span>
          </span>
          <span class="px-2 py-0.5 text-[9px] font-black uppercase tracking-widest border border-emerald-200 text-emerald-600">
      ...
      <div class="flex flex-1 overflow-hidden">
        ...
        <!-- ── Palette (left) ── -->
        <aside class="w-14 bg-[#fafafa] border-r-2 border-[#eaeded] flex flex-col items-center py-3 gap-1 flex-shrink-0 z-10">
        ...
        <!-- ── Canvas ── -->
        <section class="flex-1 relative overflow-hidden canvas-bg cursor-default"
          @dragover.prevent @drop="onCanvasDrop" @click.self="selectedNodeId = null">

          <!-- SVG edges -->
          <svg ref="edgesSvg" class="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
            <defs>
              <marker id="arr" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
                <path d="M2 1L8 5L2 9" fill="none" stroke="#d1d5db" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </marker>
            </defs>
            <path v-for="edge in computedEdges" :key="edge.id"
              :d="edge.path" fill="none" stroke="#d1d5db" stroke-width="2" marker-end="url(#arr)" />
          </svg>
          ...
          <div v-for="node in nodes" :key="node.id"
            ...
            <!-- Input port -->
            <div class="port absolute left-[-6px] top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white border-2 border-gray-300 hover:border-[#ff9900] hover:bg-amber-50 z-10 cursor-crosshair transition-colors"
              @mousedown.stop @mouseup="onPortConnect($event, node.id)"></div>

            <!-- Node header -->
            <div class="flex items-center gap-2 px-3 py-2 border-b border-gray-100">
              <div class="w-6 h-6 rounded flex items-center justify-center text-xs flex-shrink-0" :class="nodeAccent(node.type).icon">
              ...
              <span class="text-[8px] font-mono text-[#879196] bg-gray-50 px-1.5 py-0.5 rounded truncate max-w-[60px]">{{ node.vm ?? 'shared' }}</span>
            </div>

            <!-- Scripts -->
            <div class="px-2 py-1.5">
              <div v-for="script in node.scripts" :key="script.id"
                class="flex items-center gap-1.5 px-2 py-1 mb-1 rounded bg-gray-50 border border-transparent hover:border-gray-200 cursor-pointer transition-all"
                ...
                <span v-if="script.routeTo" class="text-[8px] text-gray-400 font-mono flex-shrink-0">→ {{ script.routeTo }}</span>
              </div>
              <button class="flex items-center gap-1 text-[9px] text-[#879196] hover:text-[#ff9900] transition-colors mt-1 px-2 font-black uppercase tracking-wide"
            ...
            <!-- Footer: dest bucket -->
            <div class="px-3 pb-2">
              <span class="text-[8px] font-mono text-[#d1d5db] truncate block">{{ node.destBucket ?? 's3://'+node.type+'/' }}</span>
            </div>

            <!-- Output port -->
            <div class="port absolute right-[-6px] top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white border-2 border-gray-300 hover:border-[#ff9900] hover:bg-amber-50 z-10 cursor-crosshair transition-colors"
              @mousedown.stop="startEdgeDraw($event, node.id)"></div>
          </div>

          <!-- Empty canvas hint -->
          <div v-if="nodes.length === 0" class="absolute inset-0 flex flex-col items-center justify-center text-[#d1d5db] pointer-events-none gap-3">
            <span class="text-4xl">⬡</span>
            <p class="text-[10px] font-black uppercase tracking-[0.2em]">Drag nodes from the palette to build your pipeline</p>
          </div>

          <!-- Bottom hint -->
          <div class="absolute bottom-3 left-1/2 -translate-x-1/2 text-[8px] font-black uppercase tracking-[0.2em] text-[#d1d5db] pointer-events-none whitespace-nowrap">
        ...
        <Transition name="slide-right">
          <aside v-if="selectedNode" class="w-56 bg-white border-l-2 border-[#eaeded] flex flex-col flex-shrink-0 overflow-y-auto z-10" @click.stop>

            <!-- Header -->
            <div class="px-4 py-3 border-b-2 border-[#eaeded]">
              <p class="text-[9px] font-black uppercase tracking-[0.2em] text-[#ff9900] mb-1">{{ selectedNode.type }} node</p>
              <p class="text-sm font-black text-[#232f3e] capitalize">{{ selectedNode.label ?? selectedNode.type }}</p>
            </div>
            ...
            <div class="px-4 py-3 border-b border-[#f0f0f0]">
              ...
              <div v-for="script in selectedNode.scripts" :key="script.id" class="mb-2 bg-[#fafafa] rounded p-2">
                <p class="text-[10px] font-mono text-[#232f3e] mb-1">{{ script.name }}</p>
                <div class="flex items-center gap-1 text-[9px] font-mono">
                  <span class="text-[#d1d5db]">→</span>
                  <span class="text-emerald-500 flex-1 truncate">{{ script.routeTo ?? '(default)' }}</span>
                  <button class="text-[#879196] hover:text-[#ff9900] transition-colors text-xs" @click="openScriptProps(selectedNode.id, script.id)">✎</button>
                </div>
              </div>
            ...
            <!-- VM Assignment -->
            <div class="px-4 py-3 border-b border-[#f0f0f0]">
              <p class="text-[9px] font-black uppercase tracking-[0.2em] text-[#879196] mb-2">VM Assignment</p>
              <div class="flex gap-1.5 mb-3">
                <button v-for="mode in ['shared','dedicated']" :key="mode"
                  class="flex-1 py-1.5 text-[9px] font-black uppercase tracking-wide border-2 transition-all"
                  :class="(selectedNode.vmMode===mode || (!selectedNode.vmMode && mode==='shared'))
                    ? 'border-[#ff9900] text-[#ff9900] bg-[#ff9900]/5'
                    : 'border-[#eaeded] text-[#879196] hover:border-[#ff9900]'"
                  @click="setVmMode(selectedNode.id, mode as 'shared'|'dedicated')">
                  {{ mode }}
                </button>
              </div>
              <p class="text-[9px] text-[#879196] mb-1 uppercase font-black tracking-wide">Instance</p>
              <p class="text-[9px] font-mono bg-[#fafafa] p-1.5 rounded text-[#545b64] break-all">{{ selectedNode.vm ?? 'shared-01' }}</p>
              <template v-if="selectedNode.type === 'train'">
                <div class="mt-2 flex items-center justify-between">
                  <p class="text-[9px] text-[#879196] uppercase font-black tracking-wide">VM count</p>
                  <p class="text-[9px] font-mono text-[#232f3e]">{{ selectedNode.vmCount ?? 1 }}</p>
                </div>
                <div class="mt-1 flex items-center justify-between">
                  <p class="text-[9px] text-[#879196] uppercase font-black tracking-wide">Warm standby</p>
                  <p class="text-[9px] font-mono" :class="selectedNode.warmStandby ? 'text-amber-500' : 'text-[#879196]'">
                    {{ selectedNode.warmStandby ? 'on' : 'off' }}
                  </p>
                </div>
              </template>
            </div>
            ...
            <!-- Storage -->
            <div class="px-4 py-3 border-b border-[#f0f0f0]">
              <p class="text-[9px] font-black uppercase tracking-[0.2em] text-[#879196] mb-2">Storage</p>
              <div v-if="selectedNode.srcBucket" class="mb-2">
                <p class="text-[8px] text-[#879196] uppercase font-black">Source</p>
                <p class="text-[9px] font-mono text-[#545b64] bg-[#fafafa] p-1.5 rounded break-all mt-0.5">{{ selectedNode.srcBucket }}</p>
              </div>
              <p class="text-[8px] text-[#879196] uppercase font-black">Destination</p>
              <p class="text-[9px] font-mono text-[#545b64] bg-[#fafafa] p-1.5 rounded break-all mt-0.5">{{ selectedNode.destBucket ?? 's3://'+selectedNode.type+'/' }}</p>
            </div>

            <!-- Advanced Trigger -->
            <div class="px-4 py-3 border-b border-[#f0f0f0]">
              <p class="text-[9px] font-black uppercase tracking-[0.2em] text-[#879196] mb-2">Execution Trigger</p>
              <div class="flex items-center justify-between mb-2">
                <span class="text-[9px] text-[#232f3e] font-black uppercase tracking-wide">Auto-execute</span>
                <button
                  @click="selectedNode.default_run = !selectedNode.default_run; saveProject()"
                  class="w-8 h-4 rounded-full transition-colors relative"
                  :class="selectedNode.default_run ? 'bg-[#ff9900]' : 'bg-[#eaeded]'"
                >
                  <span class="absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all" :class="selectedNode.default_run ? 'left-4' : 'left-0.5'"></span>
                </button>
              </div>
              <p class="text-[8px] text-[#879196] mb-1 uppercase font-black tracking-wide">Schedule (e.g. 1am, 1h)</p>
              <input v-model="selectedNode.schedule" @change="saveProject" placeholder="Not scheduled"
            ...
            <!-- On Failure -->
            <div class="px-4 py-3 border-b border-[#f0f0f0]">
              <p class="text-[9px] font-black uppercase tracking-[0.2em] text-[#879196] mb-1">On Failure</p>
              <p class="text-[9px] font-mono text-red-500">stop pipeline</p>
            </div>

            <!-- Delete -->
            <button class="mx-4 my-3 py-2 border border-red-200 text-red-500 text-[9px] font-black uppercase tracking-[0.15em] hover:bg-red-50 transition-all flex items-center justify-center gap-1"
        ...
        <div class="flex-1 flex items-center gap-6 px-5 overflow-hidden">
          ...
          <template v-if="activeBottomTab==='RESOURCES'">
            <div v-for="m in resourceMetrics" :key="m.label" class="flex flex-col gap-1 min-w-[100px]">
              ...
              <div class="h-3 bg-[#f0f0f0] rounded overflow-hidden">
                <div class="h-full rounded transition-all duration-500" :class="m.tw" :style="{ width: m.value+'%' }"></div>
              </div>
            </div>
            <div class="ml-auto flex gap-6">
              ...
              <div class="flex flex-col items-center">
                <span class="text-sm font-black leading-tight" :class="running ? 'text-amber-500' : 'text-emerald-500'">{{ running ? 'running' : 'idle' }}</span>
          ...
          <template v-else-if="activeBottomTab==='RUN LOG'">
            <div class="flex flex-col gap-1 flex-1 overflow-y-auto h-full py-1.5">
              ...
              <div v-if="!runLog.length" class="text-[9px] font-mono text-[#d1d5db]">no log entries yet — run the pipeline first</div>
            </div>
          </template>
          ...
          <template v-else-if="activeBottomTab==='ARTIFACTS'">
            <div class="flex flex-col gap-1.5 flex-1 overflow-y-auto h-full py-1.5">
              ...
              <div v-if="!artifacts.length" class="text-[9px] font-mono text-[#d1d5db]">run the pipeline to generate artifacts</div>
            </div>
          </template>
    ...
    <Transition name="fade">
      <div v-if="showAddNode" class="fixed inset-0 z-[200] flex items-center justify-center bg-[#232f3e]/50 backdrop-blur-sm p-6" @click.self="showAddNode=false">
        <div class="bg-white border-2 border-[#232f3e] w-full max-w-md shadow-2xl">
          <div class="flex items-center justify-between px-6 py-4 border-b-2 border-[#eaeded]">
            ...
            <button @click="showAddNode=false" class="text-[#879196] hover:text-[#232f3e] transition-colors bg-transparent border-none cursor-pointer text-lg">✕</button>
          </div>
          <div class="p-6 grid grid-cols-2 gap-3">
            <div v-for="pt in paletteTypes" :key="pt.type"
              ...
              <p class="text-[10px] font-black uppercase tracking-wider text-[#232f3e] group-hover:text-[#ff9900] transition-colors">{{ pt.label }}</p>
    ...
    <!-- ══ SCRIPT MODAL ═══════════════════════════════════════════════════════ -->
    <Transition name="fade">
      <div v-if="showScriptModal" class="fixed inset-0 z-[200] flex items-center justify-center bg-[#232f3e]/50 backdrop-blur-sm p-6" @click.self="showScriptModal=false">
        <div class="bg-white border-2 border-[#232f3e] w-full max-w-sm shadow-2xl">
          <div class="flex items-center justify-between px-6 py-4 border-b-2 border-[#eaeded]">
            <div>
              ...
              <h3 class="text-lg font-black text-[#232f3e] uppercase tracking-tighter">{{ editingScript ? 'Edit' : 'Add' }} Script</h3>
            </div>
            <button @click="showScriptModal=false" class="text-[#879196] hover:text-[#232f3e] transition-colors bg-transparent border-none cursor-pointer text-lg">✕</button>
          </div>
          <div class="px-6 py-4 space-y-4">
            <div>
              <label class="block text-[9px] font-black uppercase tracking-[0.18em] text-[#545b64] mb-1.5">Script name *</label>
              <input v-model="scriptForm.name" type="text" placeholder="e.g. parliament_scraper.py"
                class="w-full px-3 py-2.5 border-2 border-[#eaeded] text-sm text-[#232f3e] font-mono focus:outline-none focus:border-[#ff9900] transition-colors" />
            </div>
            <div>
              <label class="block text-[9px] font-black uppercase tracking-[0.18em] text-[#545b64] mb-1.5">Path / URL</label>
              <input v-model="scriptForm.path" type="text" placeholder="s3://scripts/ingest/"
                class="w-full px-3 py-2.5 border-2 border-[#eaeded] text-sm text-[#232f3e] font-mono focus:outline-none focus:border-[#ff9900] transition-colors" />
            </div>
            <div v-if="scriptFormNodeType === 'ingest'">
              ...
              <select v-model="scriptForm.routeTo" class="w-full px-3 py-2.5 border-2 border-[#eaeded] text-sm text-[#232f3e] focus:outline-none focus:border-[#ff9900] transition-colors bg-white">
                <option value="">(none — use default)</option>
                <option v-for="cs in availableCleanerScripts" :key="cs" :value="cs">{{ cs }}</option>
              </select>
            </div>
          </div>
          <div class="flex gap-3 px-6 pb-6">
            <button @click="showScriptModal=false" class="flex-1 py-2.5 border-2 border-[#232f3e] text-[#232f3e] text-[9px] font-black uppercase tracking-[0.18em] hover:bg-gray-50 transition-all">Cancel</button>
            <button @click="saveScript" :disabled="!scriptForm.name.trim()" class="flex-1 py-2.5 bg-[#ff9900] text-white text-[9px] font-black uppercase tracking-[0.18em] hover:bg-[#ec7211] transition-all disabled:opacity-40">
              {{ editingScript ? 'Save' : 'Add Script' }}
            </button>
          </div>
        </div>
      </div>
...
.port { position: absolute; top: 50%; transform: translateY(-50%); z-index: 5; }
